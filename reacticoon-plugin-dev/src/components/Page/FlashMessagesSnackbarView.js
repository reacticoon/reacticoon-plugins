...
import { FlashMessageType } from "reacticoon-plugins/reacticoon-plugin-flash-messages/src/service";
...
const FlashMessages = React.memo(({ flashMessages }) => {
  ...
  React.useEffect(() => {
    flashMessages.map((flashMessage, index) => {
      ...
      const isEventException =
        event && event.type === "REACTICOON::LOG::EXCEPTION"; // TODO: use constant
    ...
    <Card className={classes.card} ref={ref}>
      ...
      >
        <Typography variant="subtitle2" className={classes.typography}>
          {isEventException
            ? `Exception ${event.data.exceptionMessage}`
            : flashMessage.text}
        </Typography>
...
const FlashMessageSnackbarView = () => (
  <SnackbarProvider
    ...
    content={(key, flashMessage) => (
      <SnackMessage key={key} id={key} flashMessage={flashMessage} />
    )}
  >
