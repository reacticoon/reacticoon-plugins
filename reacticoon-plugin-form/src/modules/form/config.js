...
export const getFormConfig = formDef => {
  ...
  const formConfig = find(
    getForms(),
    formConfig => formType === formConfig.form.type
  );
...
export const getFormReducer = formDef => {
  ...
  invariant(
    isNil(reducer) || isFunction(reducer),
    "form " + formType + " has no reducer"
  );
...
export const getFormModifier = formDef => {
  ...
  invariant(
    isNil(modifier) || isFunction(modifier),
    "form " + formType + " has no modifier"
  );
